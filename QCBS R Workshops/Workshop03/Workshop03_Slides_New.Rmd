...
>>>>>>> 1197e62f45a16f74d49677435b1589234b2328fe
...
```
```{r, echo=FALSE}
ggplot(mpg, aes(displ, hwy)) + geom_point() + geom_smooth(method = lm)+ labs(title = "Plot")
```
]
...
```
```{r, echo=FALSE}
ggplot(data = iris, aes(Species, Sepal.Length)) + geom_violin()+ labs(title = "Violin plot")
```
]
