...
.center[Generate a plot of showing **concentration versus uptake** where each plant is shown using a different .alert[colour] point.
...
.center[.comment[`function()` itself returns the last evaluated value even without including `return()` function.]]
...
.pull-left[
.alert[Tip.] Be very careful when creating variables inside a conditionnal statement as the variable may never have been created and cause (sometimes unperceptible) errors.
<br>
<br>
<br>
...
.center[.comment[Whenever possible, avoid using names of existing `R` functions and variables to avoid confusion and conflits.]]

---
## Use comments

.alert[Final tip]. Add comment to describe what your code does, how to use its arguments or a detailed step-by-step description the function.
